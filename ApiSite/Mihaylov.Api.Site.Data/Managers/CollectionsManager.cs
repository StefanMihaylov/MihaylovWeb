...
namespace Mihaylov.Api.Site.Data.Managers
{
    public class CollectionsManager : ICollectionsManager
    {
        ...
        protected readonly Lazy<ConcurrentDictionary<int, AccountStatus>> answerTypesById;
        protected readonly Lazy<ConcurrentDictionary<string, AccountStatus>> answerTypesByName;
        ...
        protected readonly Lazy<ConcurrentDictionary<int, IEnumerable<CountryState>>> statesById;
            ...
            ILoggerFactory loggerFactory)
        {
            ...
            IEnumerable<AccountStatus> answerTypeList = new List<AccountStatus>();

            this.answerTypesById = new Lazy<ConcurrentDictionary<int, AccountStatus>>(() =>
            {
                IDictionary<int, AccountStatus> answerTypes = answerTypeList.ToDictionary(p => p.Id);
                return new ConcurrentDictionary<int, AccountStatus>(answerTypes);
            });

            this.answerTypesByName = new Lazy<ConcurrentDictionary<string, AccountStatus>>(() =>
            {
                IDictionary<string, AccountStatus> answerTypes = answerTypeList.ToDictionary(p => p.Name);
                return new ConcurrentDictionary<string, AccountStatus>(answerTypes, StringComparer.OrdinalIgnoreCase);
            });
            ...
            this.statesById = new Lazy<ConcurrentDictionary<int, IEnumerable<CountryState>>>(() =>
            {
                return new ConcurrentDictionary<int, IEnumerable<CountryState>>(stateDictionary);
            });
        }
        ...
        {
            IEnumerable<AccountStatus> answerTypes = this.answerTypesById.Value.Values;
            return answerTypes;
        }

        public AccountStatus GetAnswerTypeById(int id)
        {
            if (this.answerTypesById.Value.TryGetValue(id, out AccountStatus type))
            {
                return type;
            }
            else
        ...
        public AccountStatus GetAnswerTypeByName(string name)
        {
            if (this.answerTypesByName.Value.TryGetValue(name.Trim(), out AccountStatus type))
            {
                return type;
            }
            else
        ...
        public IEnumerable<CountryState> GetAllStatesByCountryId(int countryId)
        {
            if (this.statesById.Value.TryGetValue(countryId, out IEnumerable<CountryState> states))
            {
                return states;
            }
            else
